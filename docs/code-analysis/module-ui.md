...
**Bundle organization:** `vite.config.ts` sets `optimizeDeps: { noDiscovery: true, include: ['react', 'react-dom', 'react-dom/client', 'react-router-dom', 'recharts'] }` — `noDiscovery` disables esbuild's source-file crawling (which would otherwise trigger intermittent EIO errors reading `.tsx` files off the WSL2 bind mount at startup), and `include` explicitly pre-bundles the deps that crawling would normally have found, Recharts among them. An earlier pass of this analysis flagged a discrepancy here (config expected to pre-bundle Recharts but reportedly not doing so); the current `vite.config.ts` confirms `optimizeDeps.include` does list `recharts`, so the first request to a Recharts chart chunk does not trigger on-demand bundling — that finding is resolved.

**VITE_API_KEY exposure:** The API key is embedded in the bundled JS at build time (visible in browser DevTools Network tab, source maps, and `dist/assets/*.js`). This is a deliberate tradeoff for a self-hosted developer tool, documented as such in `docs/configuration.md` (`VITE_API_KEY` — "API key sent with every UI request as `X-API-Key`"). For multi-tenant or public-facing deployment, API key authentication would need to shift to a BFF (backend-for-frontend) session cookie pattern.
