...
| B4 | results-service | **Resolved** — previously reported as "webhook sends raw secret as X-Webhook-Secret header, no HMAC computed". Re-verified against `consumer.ts`: `fireWebhooks` computes `sha256=hmac-sha256(secret, body)` and sends `X-Webhook-Signature`, matching `docs/ARCHITECTURE.md`'s Security Architecture section. Signing now applies to any format (`generic`, `slack`, `pagerduty`, `opsgenie`) when a secret is configured | No action needed | Resolved |
