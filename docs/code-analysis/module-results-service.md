...
The webhook header is `X-Webhook-Secret` (line 45), not `X-Webhook-Signature`. The CLAUDE.md documents it as HMAC-signed, but the actual implementation passes the raw secret value as a header — it is not HMAC-computed. This is a security regression from the documented spec.
...
2. **Webhook HMAC is not implemented.** `consumer.ts:45` sends the raw secret as `X-Webhook-Secret`, not a computed HMAC. The CLAUDE.md documents `X-Webhook-Signature: sha256=<hmac>`. This gap means webhook consumers cannot verify payload authenticity.
