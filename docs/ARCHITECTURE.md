...
**Chat-driven script editing (added 2026-07-14).** `test_scripts` (the cache keyed by `UNIQUE(target_url, test_type)`, previously invisible to users — `GET /scripts`/`GET /scripts/:id` existed but nothing in the UI called them) is now surfaced via a new "Saved Scripts" page and editable through a 4th chat mode. A new `test_script_versions` table (migration 18: `id`, `script_id` FK → `test_scripts(id)` `ON DELETE CASCADE`, `script`, `created_at`) archives the *previous* script text every time `PUT /scripts/:id` or `POST /scripts/:id/versions/:versionId/restore` overwrites the current one — `test_scripts` itself is untouched structurally, so every existing cache-hit lookup (`findExistingScript`, etc.) keeps working unmodified; only the current row's `script` column changes. `POST /chat/edit-script` (`routes/ai.ts`) is the AI side: given a `scriptId` and a conversation, `buildScriptEditPrompt()` (`routes/helpers.ts`) fences the *existing* script's full text and asks the model to return a **complete** replacement (not a diff/patch) applying only the requested change — this treats editing as raw source-text transformation uniformly across backend/flow (k6) and client-side (Puppeteer) scripts, deliberately not attempting to parse a script back into structured `FlowStep[]` (the same "black box" tradeoff the platform already accepts for the `customScript` bypass). The endpoint never persists on its own — saving is a separate, explicit `PUT /scripts/:id` call from the resulting chat card, so an in-progress multi-turn edit can't accidentally overwrite the script everyone else's cache-hit lookups are still using.
...
**Saved Scripts page + chat edit mode (added 2026-07-14).** `app/scripts/page.tsx` lists `test_scripts` (finally putting `getScripts()`/`GET /scripts` to use), with a per-script expandable preview, version history (`getScriptVersions`/restore), and an "Edit via Chat" action that stores `{ scriptId }` in `sessionStorage` and navigates to `/chat` — read-and-cleared there the same way `chatFlowConfig` is read for the Flow Builder handoff. Chat's 4th mode (`edit`) either starts from that pre-loaded script or shows an in-chat picker (`getScripts()` again) when none is chosen yet; each turn calls `editScriptChat()` → `POST /chat/edit-script` instead of `parseChatPrompt()`, and the resulting card's **Save** button is the only thing that actually persists the edit (`saveScript()` → `PUT /scripts/:id`) — sending more messages before saving never touches the stored script.
