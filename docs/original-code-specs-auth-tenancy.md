...
**[INCONSISTENCY — correction, 2026-07-06]:** results-service has an extensive `internalPaths` + `internalSuffixes` exemption set (A.3); api-service only exempts `/health` plus the new `X-Internal-Key` trusted-caller path. `POST /tests/:testId/cancel` does require normal session/API-key auth to reach api-service — but the previous note's conclusion ("this remains correct behavior, not a bug") was wrong. Verified against current source: api-service's handler (`index.ts:328-343`) authenticates the *caller* but performs no check that the `testId` in the URL belongs to `request.projectId` before forwarding to results-service's `/results/:testId/cancel` via the trusted internal channel; results-service's handler (`routes/results.ts:98-113`) does `UPDATE test_results SET status='cancelled' ... WHERE test_id = $1 AND status IN ('pending','running')` with no `project_id` filter either. The combination means any authenticated user of *any* team can cancel *any other* team's running test by supplying its `testId` — confirmed as a Critical finding in `TODO.md`'s security-review pass. "Requires session auth" only proves the caller is a legitimate user of the platform; it says nothing about whether they have rights over the specific resource they're targeting, which is the actual isolation property this endpoint is missing.
