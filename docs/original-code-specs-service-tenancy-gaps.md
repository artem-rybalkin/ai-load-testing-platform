...
|----|------|-------------|----------|
| X-U1 | Unknown | `results-service/src/consumer.ts` was not read in this analysis. It is cited in Phase 1 specs (`reference-code-specs-project-isolation.md:173`) as saving `projectId` to `test_results.project_id`. That claim is assumed correct based on Phase 1 output, not re-verified here. | Phase 1 spec section 6 |
