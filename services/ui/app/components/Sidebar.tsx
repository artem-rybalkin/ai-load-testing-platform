...
export default function Sidebar() {
  ...
  // Below lg (tablet width, md:–lg:), always force the narrow icon-rail —
  // there isn't enough room to justify the user's lg+ expand/collapse preference.
  const isLgUp = useMediaQuery('(min-width: 1024px)');
  const effectiveOpen = isLgUp && open;
  ...
  return (
    <aside
      className="hidden md:flex flex-col flex-shrink-0 border-r border-[#d0d7de] dark:border-[#30363d] sticky top-0 h-screen overflow-y-auto transition-[width] duration-150 ease-in-out"
      style={{ width: effectiveOpen ? 220 : 48, background: 'var(--surface)', color: 'var(--text-primary)' }}
    >
      {/* Logo + toggle */}
      ...
      {/* Dark mode toggle */}
      <div className={`flex items-center border-b border-[#d0d7de] min-h-[40px] ${effectiveOpen ? 'px-3' : 'justify-center'}`}>
        <button
          onClick={toggleDark}
        ...
        >
          ...
          {effectiveOpen && <span>{dark ? 'Light mode' : 'Dark mode'}</span>}
        </button>
      </div>
      ...
      {/* User + Logout */}
      {user && (
        <div className={`border-t border-[#d0d7de] p-2 ${effectiveOpen ? '' : 'flex justify-center'}`}>
          {effectiveOpen && (
          ...
          <button
            ...
            title={effectiveOpen ? undefined : 'Sign out'}
            className={`flex items-center gap-2.5 px-2.5 py-2 rounded-md text-[13px] text-[#57606a] hover:bg-[#f6f8fa] hover:text-[#cf222e] transition-colors w-full ${effectiveOpen ? '' : 'justify-center'}`}
          >
            <span className="text-[15px] w-5 text-center flex-shrink-0">⏻</span>
            {effectiveOpen ? <span className="truncate">Sign out</span> : <span className="sr-only">Sign out</span>}
          </button>
        </div>
      )}
    </aside>
