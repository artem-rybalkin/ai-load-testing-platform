...
// ─── Queue consumer — retry/DLQ routing when handleResult throws (M5) ────────
//
// Inline re-implementation of the channel.consume(QUEUE, ...) wrapper in
// src/consumer.ts's startConsumer() (the try/catch around handleResult that
// increments x-retry-count and republishes, or routes to the DLQ once
// MAX_RETRIES is exhausted) — same pattern already used in
// worker-backend/__tests__/index.test.ts and ai-service's processor.ts split.
// No real queue/broker needed: only the error-propagation + retry math matters.

const CONSUMER_QUEUE       = 'test-results';
...
const makeConsumerMsg = (retryCount?: number): { content: Buffer; properties: { headers: Record<string, number> } } => ({
  content: Buffer.from(JSON.stringify(makeResult())),
  properties: { headers: retryCount !== undefined ? { 'x-retry-count': retryCount } : {} },
});

/** Matches the try/catch body of the QUEUE consumer in src/consumer.ts exactly. */
const consumeHandlerFn = async (
  channel: ReturnType<typeof makeConsumerChannel>,
  msg: ReturnType<typeof makeConsumerMsg>,
  runHandleResult: () => Promise<void>,
): Promise<void> => {
  try {
    await runHandleResult();
    channel.ack(msg as never);
  } catch {
    const retryCount = Number(msg.properties.headers?.['x-retry-count'] ?? 0);
    if (retryCount < CONSUMER_MAX_RETRIES) {
      channel.publish('', CONSUMER_QUEUE, msg.content, {
        persistent: true,
        headers: { ...msg.properties.headers, 'x-retry-count': retryCount + 1 },
      });
    } else {
      channel.sendToQueue(CONSUMER_DLQ, msg.content, { persistent: true });
    }
    channel.ack(msg as never);
  }
};

describe('Queue consumer — retry/DLQ routing when handleResult throws (M5)', () => {
  it('propagates the handleResult error and republishes with incremented x-retry-count when below MAX_RETRIES', async () => {
    const channel = makeConsumerChannel();
    const msg     = makeConsumerMsg(0);

    await consumeHandlerFn(channel, msg, async () => { throw new Error('INSERT failed'); });

    expect(channel.publish).toHaveBeenCalledWith(
      '', CONSUMER_QUEUE, msg.content,
  ...
  it('routes to the DLQ once MAX_RETRIES is exhausted', async () => {
    const channel = makeConsumerChannel();
    const msg     = makeConsumerMsg(CONSUMER_MAX_RETRIES);

    await consumeHandlerFn(channel, msg, async () => { throw new Error('INSERT failed'); });

    expect(channel.sendToQueue).toHaveBeenCalledWith(CONSUMER_DLQ, msg.content, { persistent: true });
  ...
  it('routes to the DLQ when retryCount exceeds MAX_RETRIES', async () => {
    const channel = makeConsumerChannel();
    const msg     = makeConsumerMsg(CONSUMER_MAX_RETRIES + 5);
  ...
  it('treats a missing x-retry-count header as 0 (first attempt)', async () => {
    const channel = makeConsumerChannel();
    const msg     = makeConsumerMsg(); // no header
  ...
  it('acks without retrying or DLQ-routing when handleResult succeeds', async () => {
    const channel = makeConsumerChannel();
    const msg     = makeConsumerMsg(0);

    await consumeHandlerFn(channel, msg, async () => { /* success */ });
